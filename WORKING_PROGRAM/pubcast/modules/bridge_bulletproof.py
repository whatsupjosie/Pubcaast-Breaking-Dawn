...
class VoxelBridge:
    ...
    # ----------------------------------------------------------------
    # V3 constants — match bridge.h exactly
    # ----------------------------------------------------------------
    _V3_BRIDGE_NAME    = "Local\\PubCast_V3_Bridge"
    _V3_MAGIC          = 0x50554243          # "PUBC"
    _V3_VERSION        = 300
    _V3_SHM_SIZE       = 1024 * 1024 * 8    # 8 MB
    # Header offsets (alignas(64) struct, standard C++ padding)
    _V3_OFF_STRESS     = 8    # float   engine_stress
    _V3_OFF_BACKLOG    = 12   # uint32  backlog
    _V3_OFF_ENG_PULSE  = 16   # uint64  engine_pulse
    _V3_OFF_ART_PULSE  = 24   # uint64  artist_pulse
    _V3_OFF_DELTA      = 32   # double  last_frame_delta
    _V3_OFF_HEAD       = 40   # uint32  head  (C++ owns — never write)
    _V3_OFF_TAIL       = 44   # uint32  tail  (Python writes)
    _V3_QUEUE_OFF      = 64   # CommandSlot queue[1024]
    _V3_SLOT_SIZE      = 32   # bytes per CommandSlot
    _V3_QUEUE_DEPTH    = 1024
    _V3_CMD_FMT        = "<I fff fff I"  # little-endian, 32 bytes

    def _connect_windows_v3(self) -> bool:
        """
        Path A (Windows) — open the C++ engine's named file mapping.
        Uses mmap tagname= which maps to Windows CreateFileMapping/OpenFileMapping.
        Falls through cleanly if the engine isn't running or we're not on Windows.
        """
        try:
            self._v3_shm = mmap.mmap(
                -1, self._V3_SHM_SIZE,
                tagname=self._V3_BRIDGE_NAME,
                access=mmap.ACCESS_WRITE,
            )
            magic, version = struct.unpack_from("<II", self._v3_shm, 0)
            if magic != self._V3_MAGIC or version != self._V3_VERSION:
                logger.warning(
                    "🌉 V3 bridge found but signature mismatch "
                    "(magic=0x%08X version=%d) — engine may still be starting",
                    magic, version,
                )
                self._v3_shm.close()
                self._v3_shm = None
                return False
            logger.info(
                "✅ V3 Windows SHM bridge connected — magic=PUBC version=%d", version
            )
            return True
        except (AttributeError, OSError):
            # AttributeError: mmap.mmap has no tagname on this platform (Linux/macOS)
            # OSError: engine not running / mapping not created yet
            return False
        except Exception as exc:
            logger.debug("🌉 V3 Windows SHM probe failed: %s", exc)
            return False

    def _v3_ping(self, pulse: int) -> None:
        """Write our heartbeat into the V3 SHM header."""
        if self._v3_shm:
            struct.pack_into("<Q", self._v3_shm, self._V3_OFF_ART_PULSE, pulse)

    def _v3_metrics(self) -> dict:
        """Read Pete's live telemetry from the V3 SHM header."""
        if not self._v3_shm:
            return {}
        stress,  = struct.unpack_from("<f", self._v3_shm, self._V3_OFF_STRESS)
        backlog, = struct.unpack_from("<I", self._v3_shm, self._V3_OFF_BACKLOG)
        pulse,   = struct.unpack_from("<Q", self._v3_shm, self._V3_OFF_ENG_PULSE)
        delta,   = struct.unpack_from("<d", self._v3_shm, self._V3_OFF_DELTA)
        return {"stress": stress, "backlog": backlog,
                "engine_pulse": pulse, "frame_delta": delta}

    def _v3_push_command(self, cmd_type: int,
                         x=0.0, y=0.0, z=0.0,
                         rx=0.0, ry=0.0, rz=0.0,
                         target_id: int = 0) -> bool:
        """Write one CommandSlot to the V3 ring buffer and advance tail."""
        if not self._v3_shm:
            return False
        head, = struct.unpack_from("<I", self._v3_shm, self._V3_OFF_HEAD)
        tail, = struct.unpack_from("<I", self._v3_shm, self._V3_OFF_TAIL)
        # Sentinel full-check
        if (tail + 1) % self._V3_QUEUE_DEPTH == head % self._V3_QUEUE_DEPTH:
            return False
        slot   = tail % self._V3_QUEUE_DEPTH
        offset = self._V3_QUEUE_OFF + slot * self._V3_SLOT_SIZE
        payload = struct.pack(self._V3_CMD_FMT,
                              cmd_type, x, y, z, rx, ry, rz, target_id)
        self._v3_shm[offset:offset + self._V3_SLOT_SIZE] = payload
        # Advance tail — head belongs to C++, never touch it
        struct.pack_into("<I", self._v3_shm, self._V3_OFF_TAIL, tail + 1)
        return True

    def _connect_shared_memory(self, create_shared: bool = False) -> bool:
        """Path A — try Windows V3 named mapping first, fall back to POSIX SHM."""
        # Windows: try the C++ engine's named file mapping
        if not hasattr(self, "_v3_shm"):
            self._v3_shm = None
        if self._connect_windows_v3():
            return True

        # Non-Windows or engine not running: try posix_ipc
        try:
            import posix_ipc
    ...
    async def _send_via_shm(self, cmd_data: Dict[str, Any]) -> bool:
        # ── V3 Windows path: binary ring buffer ──────────────────────────────
        if getattr(self, "_v3_shm", None) is not None:
            try:
                ct = cmd_data.get("type", 0)
                pl = cmd_data.get("payload", {})
                pos = pl.get("position", {}) if isinstance(pl.get("position"), dict) else {}
                rot = pl.get("rotation", {}) if isinstance(pl.get("rotation"), dict) else {}
                ok = self._v3_push_command(
                    cmd_type  = int(ct),
                    x  = float(pos.get("x", 0)),
                    y  = float(pos.get("y", 0)),
                    z  = float(pos.get("z", 0)),
                    ...
                    target_id = int(pl.get("target_id", 0)),
                )
                if ok:
                    self._metrics.bytes_transferred += self._V3_SLOT_SIZE
                    # Feed real engine stress into IRM so E-Pete has live data
                    m = self._v3_metrics()
                    if m and m.get("frame_delta", 0) > 0:
                        self._irm_sensor.record_frame(m["frame_delta"])
                return ok
            except Exception as exc:
                logger.error("🌉 V3 SHM send failed: %s", exc)
                return False

        # ── POSIX SHM path (Linux / legacy) ──────────────────────────────────
